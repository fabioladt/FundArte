<body data-spy="scroll" data-target=".navbar-collapse">
<!-- navigation -->
<div class="navbar navbar-default navbar-fixed-top" role="navigation">
	<div class="container">
		<div class="navbar-header">
			<button class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
				<span class="icon icon-bar"></span>
				<span class="icon icon-bar"></span>
				<span class="icon icon-bar"></span>
			</button>

			<a href="<?=base_url()?>home2" ><img src="<?=base_url()?>public/images/logo.jpg" ></a>
		</div>
		<div class="collapse navbar-collapse">
			<ul class="nav navbar-nav navbar-right">
                <li><a href="#nosotros" class="smoothScroll"><b>Solar</b></a></li>
                <li><a href="#empresa" class="smoothScroll"><b>Cafeto</b></a></li>
                <li><a href="#nosotros" class="smoothScroll"><b>Aprendizajes</b></a></li>
                <li><a href="#empresa" class="smoothScroll"><b>Voluntariado</b></a></li>
                <li><a href="#aspirante" class="smoothScroll"><b>Donaciones</b></a></li>
                <li><a href="#aspirante" class="smoothScroll"><b>Usuarios</b></a></li>
			</ul>
		</div>
	</div>
</div>
